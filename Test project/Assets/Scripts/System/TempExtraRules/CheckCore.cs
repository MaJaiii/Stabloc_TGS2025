...
public class CheckCore : MonoBehaviour
{
    ...
    public Vector3[] CheckFill()
    {
        ...
        if (pos.x == origins.x && pos.z == origins.y)     //Position 0(-2,-2)
        {
            ...
            //Vertical
            hits0 = Physics.RaycastAll(rayFromThisToDown, Mathf.Infinity, 1 << 8);
            foreach (RaycastHit hit0 in hits0) if (hit0.transform.name != this.name)
                {
                    ...
                    foreach (RaycastHit hit1 in hits1)
                    {
                        ...
                        if (Mathf.Abs(Vector3.Magnitude(pos - hit1CC.pos) - 4) < .5f)
                        {
                            RaycastHit[] hits2 = Physics.RaycastAll(hit1.transform.position, Vector3.down, Mathf.Infinity, 1 << 8);
                            foreach (RaycastHit hit2 in hits2)
                            {
                                CheckCore hit2CC = hit2.transform.GetComponentInChildren<CheckCore>();
                                if (Mathf.Abs(Vector3.Magnitude(hit2CC.pos - hit1CC.pos) - 4) < .5f)
                                {
                                    result = new Vector3[4] { pos, hit0CC.pos, hit1CC.pos, hit2CC.pos };
                                    for (int i = 0; i < result.Length; i++) result[i].y = Mathf.Max(this.transform.position.y, hit0.transform.position.y, hit1.transform.position.y, hit2.transform.position.y);
                                    return result;
                                }
                            }
                        }
                    }
                    hits1 = Physics.RaycastAll(rayFromThisToForward, 6, 1 << 8);
                    foreach (RaycastHit hit1 in hits1)
                    {
                        CheckCore hit1CC = hit1.transform.GetComponentInChildren<CheckCore>();
                        if (Mathf.Abs(Vector3.Magnitude(pos - hit1CC.pos) - 4) < .5f)
            ...
            foreach (RaycastHit hit0 in hits0) if (hit0.transform.name != this.name)
                {
                    ...
                    hits1 = Physics.RaycastAll(rayFromThisToBack, 6, 1 << 8);
                    foreach (RaycastHit hit1 in hits1)
                    {
                        CheckCore hit1CC = hit1.transform.GetComponentInChildren<CheckCore>();
                        if (Mathf.Abs(Vector3.Magnitude(pos - hit1CC.pos) - 4) < .5f)
