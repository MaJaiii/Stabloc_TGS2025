...
public class ScoreSystem : MonoBehaviour
{
	...
	bool finishedTutorial = false; // チュートリアルが終わったか判定 
								   // -------------------------------------------- 

	private void Start()
	{
		actionTimer = GetComponent<ActionTimer>();
		nowScore.enabled = false;
	...
	private void Update()
	{
		...
		if (blockAction.tutorial) return;
		if (!nowScore.enabled && !finishedTutorial)
		{
			nowScore.enabled = true;
			finishedTutorial = true;
		}
		if (gameOver.isGameOver && !isGameOver)
		{
			isGameOver = true;
			Destroy(shiningScore.gameObject);
			Destroy(nowScore.gameObject);
			return;
		}
		...
		// 追加-----------------------------------------
		// テキストを光らせるか判定
		if (score >= multipleCount + textMultiple)
		{
			Debug.Log($"{prevScore} : {targetScore}");
			// 最後の光らせるスコアを計算する（連続呼び出し防止）
			multipleCount = score - (score % textMultiple);
	...
	IEnumerator SwitchScoreDisplay()
	{
		...
		yield return new WaitUntil(() => prevScore >= multipleCount);
